...
//      }()
    ...
    override func viewDidLoad() {
        ...
        let newChatBtn = UIButton()
        newChatBtn.setImage(UIImage(named: "addChat"), for: .normal)
        newChatBtn.frame = CGRect(x: self.view.frame.size.width/2 + 45, y: self.view.frame.size.height/2 + 250, width: 60, height: 60)
        newChatBtn.tintColor = #colorLiteral(red: 0.9960784314, green: 0.5960784314, blue: 0, alpha: 1)
        newChatBtn.addTarget(self, action: #selector(addButtonPressed), for: .touchUpInside)
        view.addSubview(newChatBtn)
        
        definesPresentationContext = true

        navigationItem.backBarButtonItem = UIBarButtonItem(title: "", style: .plain, target: self, action: nil)
        navigationItem.backBarButtonItem?.tintColor = #colorLiteral(red: 0.9960784314, green: 0.5960784314, blue: 0, alpha: 1)
...
//     }
    ...
    @objc private func addButtonPressed() {
        let alertController = UIAlertController(title: "새로운 채팅", message: nil, preferredStyle: .alert)
